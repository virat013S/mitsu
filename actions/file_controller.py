...
from actions.mitsu_file_stamp import write_sidecar_metadata, write_text_with_stamp
...
def create_file(path: str, name: str = "", content: str = "") -> str:
    try:
        ...
        write_text_with_stamp(
            target,
            content,
            f"Created through MITSU file_controller create_file action for {target.name}.",
        )
        ...
        if src.is_dir():
            ...
            for child in dst.rglob("*"):
                if child.is_file() and not child.name.endswith(".mitsu_meta.json"):
                    write_sidecar_metadata(child, f"Copied by MITSU from {src}.")
        else:
            shutil.copy2(str(src), str(dst))
            write_sidecar_metadata(dst, f"Copied by MITSU from {src}.")
    ...
    try:
        ...
        if not append or not existed_before:
            write_text_with_stamp(
                target,
                content,
                f"Written through MITSU file_controller write action for {target.name}.",
            )
