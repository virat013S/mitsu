...
def _resolve_save_path(output_path: str, language: str) -> Path:
    ...
    return DESKTOP / f"mitsu_code{ext}"
...
def _save_file(path: Path, content: str, context: str = "Generated by MITSU code_helper.") -> str:
    try:
...
def _take_screenshot() -> Path | None:
    try:
        import pyautogui
        screenshot_path = Path.home() / "Desktop" / f"mitsu_debug_{int(time.time())}.png"
        screenshot = pyautogui.screenshot()
        screenshot.save(str(screenshot_path))
        mark_created_file(
            screenshot_path,
            "Screenshot captured by MITSU code_helper for screen debugging.",
        )
...
Updated code:"""
    ...
    status = _save_file(Path(file_path), edited, f"Edited existing code file through MITSU: {instruction}")
