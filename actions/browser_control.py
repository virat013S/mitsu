...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = home / ".mitsu_profiles" / browser
...
class _BrowserSession:
    ...
    async def _launch(self):
        ...
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".mitsu_profiles" / "safari")
        ...
        mitsu_profile = str(Path.home() / ".mitsu_profiles" / self.browser_name)
        Path(mitsu_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with MITSU profile: {mitsu_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(mitsu_profile, **kwargs)
            await asyncio.sleep(0.5)
            self._page = await self._context.new_page()
            print(f"[Browser] ✅ Launched [{label}] with MITSU profile")
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(Path.home() / "Desktop" / "mitsu_screenshot.png")
