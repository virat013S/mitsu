...
from actions.mitsu_file_stamp import write_text_with_stamp
...
PROJECTS_DIR     = Path.home() / "Desktop" / "MitsuProjects"
MAX_FIX_ATTEMPTS = 5
# NOTE: planner/writer models resolve per-call via core.llm (user
# selection), not from constants here.
...
Code for {file_path}:"""
    ...
    try:
        ...
        write_text_with_stamp(
            full_path,
            code,
            f"Generated by MITSU dev_agent for project: {project_description}. File purpose: {file_desc}",
        )
...
Fixed code for {fix_path}:"""
        ...
        try:
            ...
            write_text_with_stamp(
                full_path,
                fixed,
                f"Auto-fixed by MITSU dev_agent after build error for project: {project_description}",
            )
...
def _build_project(
    ...
    proj_name    = project_name or plan.get("project_name", "mitsu_project")
