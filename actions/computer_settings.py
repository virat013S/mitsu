...
def _is_mitsu_self_shutdown_request(params: dict, action: str, description: str, value) -> bool:
    text = f"{action} {_power_target_text(params, description, value)}"
    if "mitsu" not in text:
        return False
...
- If the user says to shut down, restart, reboot, close, quit, exit, or stop MITSU, do not choose shutdown or restart.
...
def computer_settings(
    ...
    if _is_mitsu_self_shutdown_request(params, action, description, value):
        return "MITSU self-shutdown is disabled. No computer shutdown was performed."
